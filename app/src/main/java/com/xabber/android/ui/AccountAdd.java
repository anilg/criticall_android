...
public class AccountAdd extends ManagedActivity implements
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        System.out.println("*********** entered in onActivityResult of onActivityResult **********");
	...
	public void onClick(View view) {
		...
		case R.id.ok:
			...
			if (accountType.getProtocol().isOAuth()) {
				...
				String account;
                String userName = userView.getText().toString() + "@" + getString(R.string.account_default_domain);
				try {
					...
					account = AccountManager.getInstance().addAccount(
							...
							storePasswordView.isChecked(),
							useOrbotView.isChecked());
                    System.out.println("entered in try block111111");
				} catch (NetworkException e) {
