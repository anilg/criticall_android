...
public class ContactList extends ManagedListActivity implements
	...
	public void onClick(View view) {
        System.out.println("1111111111111 entered in onClick of contactLIst");
		...
		case R.id.button: // Hint button
			...
			case R.string.application_action_empty:
                System.out.println("2222222 entered in case stmt");
		...
		default:
			...
			if (!SettingsManager.contactsShowAccounts()) {
				if (AccountManager.getInstance().getAccounts().size() < 2) {
					...
					rebuildAccountToggler();
                    System.out.println("############ entered in contactListAdapter.onChange()");
					contactListAdapter.onChange();
					stopMovement();
				}
			} else {
	...
	public void onAccountsChanged(Collection<String> accounts) {
        System.out.println("^^^^^^^^^^^ entered in onAccountsChanged");
